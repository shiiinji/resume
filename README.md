...
- 2013年からの2年間は初の自社サービスであるフォトブックサービスの立ち上げに従事しました。
...
#### インドのラボ型オフショア開発企業
   ...
   - エンジニア: 3名
   - 期間: 2013年1月～現在 
     - 価値観や職業観の違いからマネジメントは苦労がありましたが、様々な施策を経て、最終的にはタスクを細かく分割したり、レガシープロジェクトを最新のスタックに変更する部分を中心にアサインすることである一定の成果を安定的に出せるようになりました。
...
| 3 | 2021/4<br />〜<br />現在 | 運営中・新規プロジェクトを横断してものづくり産業に関わる企業様が利用できるSaaSサービス開発中しております。*半育休中しながらの開発期間を含みます。 | 1名 (本人含む)
| |
...
| # | 期間 | 内容 | 開発チーム | 
| --- | --- | --- | --- |
| 1 | 2017/3<br />〜<br />2017/4 | 立山黒部アルペンルートの単発イベント用のWebアプリとして始動しました。当初はリリースまでの時間がなかったこと、また継続的にクライアントが増えるか不明だったために実用最小限のポストカード作成とデータの蓄積/エクスポートに重点を置き開発しました。ログイン機能を持たせないことになったため、localStorage/redux-persistの同期部分で苦戦しました。 | 1名 (本人)
...
| 3 | 2019/5<br />〜<br />2019/9 | ポストカードのクライアントの増加と印刷会社と提携し共同運用し販売を拡大する方向になったため、クライアントアドミンを開発し、誰でもイベントを作成できるようにしました。困難だったところとしては、アンケート作成機能になります。アンケートは質問の数、回答方法、複数言語と考慮する範囲が広かったからです。 | 1名 (本人含む)
| |
...
### 【自社開発】 Facebook/Instagramからフォトブックサービス (https://happylogue.com/)
...
| 2 | 2013/3<br />〜<br />2013/12 | インドチームとのプロダクト開発、印刷会社とのオペレーションフローの策等のサービスリリースに向けたことを行いました。 | 4名 (本人含む)
...
- 新しい技術は積極的に取り入れるが、自社の技術スタックを分散せずに長期的な運用を考慮する
- イージーよりシンプルを好む
